<template>
  <div class="deep-background footer">
    <div class="container">
      <ul class="footer-list">
        <li><a href="#">{{$t("FooterAbout.title")}}</a></li>
        <li><a href="#">{{$t("FooterAbout.summary")}}</a></li>
        <li><a href="#">{{$t("FooterAbout.characteristic")}}</a></li>
        <li><a href="#">{{$t("FooterAbout.service")}}</a></li>
        <li><a href="#">{{$t("FooterAbout.information")}}</a></li>
      </ul>
      <ul class="footer-list ">
        <li><a href="#">{{$t("FooterHelp.title")}}</a></li>
        <li><a href="#">{{$t("FooterHelp.purchase")}}</a></li>
        <li><a href="#">{{$t("FooterHelp.payment")}}</a></li>
        <li><a href="#">{{$t("FooterHelp.invoice")}}</a></li>
        <li><a href="#">{{$t("FooterHelp.distribution")}}</a></li>
      </ul>
      <ul class="footer-list">
        <li><a href="#">{{$t("FooterLink.international")}}</a></li>
        <li><a href="#">{{$t("FooterLink.mall")}}</a></li>
        <li><a href="#">{{$t("FooterLink.leather")}}</a></li>
        <li><a href="#">{{$t("FooterLink.wood")}}</a></li>
      </ul>
      <ul class="footer-list">
        <li><a href="#">{{$t("FooterContact.title")}}</a></li>
        <li class="qr-code"></li>
        <li><a href="#">{{$t("FooterContact.phone")}}</a></li>
        <li><a href="#">{{$t("FooterContact.email")}}</a></li>
      </ul>
    </div>
    <div class="container">
      <hr />
      <p class="record">{{$t("Record")}}</p>
    </div>
  </div>

</template>

<script>
    export default {

    }
</script>

